...
const server = http.createServer(async (req, res) => {
  ...
  if (m === "POST" && url === "/api/prompt") { await readBody(req); dev.aiDoneAt = Date.now() + 1500; dev.aiJobs = (dev.aiJobs | 0) + 1; return accepted(res); }
  if (m === "GET" && url === "/api/prompt/status") { const busy = !!dev.aiDoneAt && Date.now() < dev.aiDoneAt; return send(res, 200, { busy, jobs: dev.aiJobs | 0, lastOk: true, lastError: "" }); }

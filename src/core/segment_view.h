...
struct SegmentView {
    ...
    SegmentView() : leds(nullptr), length(0), reversed(false) {}
    
    // Construct view from LED array
    SegmentView(CRGB* ledArray, uint16_t start, uint16_t len, bool rev = false)
        : leds(ledArray + start)
        , length(len)
        , reversed(rev) {}
    ...
    void fill(CRGB color, uint16_t start, uint16_t count) {
        if (start < length) {
            uint16_t actualCount = min(count, (uint16_t)(length - start));
            fill_solid(leds + start, actualCount, color);
        }
    }
    ...
    void blur(uint8_t amount) {
        blur1d(leds, length, amount);
    }
    
    void blend(CRGB* source, fract8 amount) {
        nblend(leds, source, length, amount);
    }
    
    // Fill with gradient (respects reversal)
    void gradient(CRGB startColor, CRGB endColor) {
        if (reversed) {
            fill_gradient_RGB(leds, length, endColor, startColor);
        } else {
            fill_gradient_RGB(leds, length, startColor, endColor);
        }
    }
    ...
    void rainbow(uint8_t startHue, uint8_t deltaHue = 5) {
        if (reversed) {
            fill_rainbow(leds, length, startHue + (deltaHue * length), -deltaHue);
        } else {
            fill_rainbow(leds, length, startHue, deltaHue);
        }
    }
    ...
    // Get raw pointer (for direct FastLED calls)
    CRGB* raw() { return leds; }
    const CRGB* raw() const { return leds; }
    
    // Get length
    uint16_t size() const { return length; }
    
    // Check if valid
    bool valid() const { return leds != nullptr && length > 0; }
